...
def handleSuggestion():
    ...
    while (again == True) and (clientsMessage != "No Matches for this client"):
        ...
        if ((suggestionHelpMessage != "") or (suggestionHelpMessage != None)) and (
                "is suggesting" not in suggestionHelpMessage) and (("Next" not in suggestionHelpMessage)):
            again = False
    ...
    msg = "\n SUGGEST " + suggestPersonInput + " in the " + roomsToNames.get(
        suggestRoomInput) + " with the " + suggestWeaponInput + "\n"
...
def gameBoardScreen():
    ...
    b_y = height - 256 + (50 / 2)
    b_move = Button(button_type_1, (b_x, b_y), "Move", button_font, base_color=button_color, hovering_color=button_greyed_color)
    ...
    buttons = [b_move, b_suggest, b_accuse, b_show, b_end, t_names, t_kitchen, t_ballroom, t_billiard, t_candle, t_conservatory,
           t_dagger, t_dining, t_hall, t_library, t_lounge, t_mustard, t_peacock, t_pipe, t_plum, t_candle, t_scarlet, t_study,
           t_weapons, t_white, t_wrench, t_rope]
...
t_cards = Text((t_x_cards, t_y_cards), "initial", notification_font, info_font_bold, notification_color,
               notification_color)
...
t_notification = Text((t_x_notification, t_y_notification), "Initial", notification_font, info_font_bold,
...
t_notification2 = Text((t_x_notification2, t_y_notification2), "Initial", notification_font, info_font_bold,
                       notification_color, notification_color)
...
def updateNotifications(text1, text2, text3, text4):
    ...
    t_you.update(screen)
    t_cards = Text((t_x_cards, t_y_cards), text2, notification_font, info_font_bold, notification_color,
                   notification_color)
    t_cards.update(screen)
    t_notification = Text((t_x_notification, t_y_notification), text3, notification_font, info_font_bold,
                          notification_color, notification_color)
    t_notification.update(screen)
    t_notification2 = Text((t_x_notification2, t_y_notification2), text4, notification_font, info_font_bold,
                           notification_color, notification_color)
    t_notification2.update(screen)
...
quit_btn.changeColor(pos)
quit_btn.update(SCREEN)
# buttons to choose number of players
...
while not done:
    ...
    # if the player is player 1, show No. Player option screen; otherwise show alternative screen - how to get player number from server
    ...
    for event in pygame.event.get():  # User did something
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if canClickButtons:
                ...
                if numPlayer_btn[0].isOver(pos, 200, 50):
                    message = "3"
                    s.send(message.encode())
                    print("3 clicked")
                if numPlayer_btn[1].isOver(pos, 200, 50):
                    message = "4"
                if numPlayer_btn[2].isOver(pos, 200, 50):
                    message = "5"
                if numPlayer_btn[3].isOver(pos, 200, 50):
                    message = "6"
                if quit_btn.isOver(pos, 200, 50):
                    ...
                    # pygame.quit()
                    # sys.exit()
                    b_move = game_buttons[0]
                    b_suggest = game_buttons[1]
                    b_accuse = game_buttons[2]
                    b_show = game_buttons[3]
                    b_end = game_buttons[4]
                ...
                if buttonsClicked == True:
                    if (p.hasMoved == False):
                        ...
                        blockedOptions = (
                                    moveInput == "Your move options are all blocked. You can either make an accusation or end your turn.")
                        if (moveInput != "You can only move once per turn.") and ("room" not in moveInput) and (
